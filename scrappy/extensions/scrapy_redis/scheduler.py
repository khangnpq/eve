...
QUEUE_KEY = 'scrappy:%(spider)s:%(group)s:requests'
QUEUE_CLASS = 'scrapy_redis.queue.SpiderPriorityQueue'
DUPEFILTER_KEY = 'scrappy:%(spider)s:%(group)s:dupefilter'
IDLE_BEFORE_CLOSE = 0
...
class Scheduler(object):
    ...
    def __init__(self, server, persist, queue_key, queue_cls, dupefilter_key, idle_before_close):
        ...
        self.retry_queuea_key = queue_key + "_retry"
    ...
    def open(self, spider):
        ...
        if hasattr(spider, 'tag'):
            self.df = RFPDupeFilter(self.server,
                                    self.dupefilter_key % {'spider': spider.name, 'group': spider.tag or 'default'})
        else:
            self.df = RFPDupeFilter(self.server, self.dupefilter_key % {'spider': spider.name, 'group': 'default'})
